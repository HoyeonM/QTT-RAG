...
#     except:
#         return True  # Se a detecção falhar, considerar como não-português
...
#         "Anna pisteet jokaiselle kriteerille välillä 0.0–5.0 yhdellä desimaalilla. "
#         "Vastaa vain JSON-muodossa ilman mitään lisäselityksiä tai kommentteja. "
...
#         f"[|user|]Alkuperäinen teksti (englanti): {original_doc_text}\n\nKäännös (suomi): {translated_doc_text}[|endofturn|]\n"
#         f"[|assistant|]"
#     )
...
#     system_instruction = (
#         "다음 영어 원문과 한국어 번역문의 품질을 세 가지 기준(의미론적 일치성, 문법적 정확성, 자연스러움과 유창성)에 대해 각각 0.0점에서 5.0점 사이의 소수점 첫째 자리까지의 점수로 평가해주세요.(예: 4.3)."
#         "다른 설명 없이 JSON 형식으로만 응답해주세요."
#         "예시:\n{\"의미론적 일치성\": 5.0, \"문법적 정확성\": 2.5, \"자연스러움과 유창성\": 4.3}"

#     )
#     prompt = (
#         f"[|system|]{system_instruction}[|endofturn|]\n"
#         f"[|user|]영어 원문: {original_doc_text}\n\n 한국어 번역문: {translated_doc_text}[|endofturn|]\n"
#         f"[|assistant|]"
#     )
...
def filter_translation_quality_with_llm(
    ...
    system_instruction = (
        "يرجى تقييم جودة الترجمة بين النص الأصلي باللغة الإنجليزية وترجمته إلى العربية "
        "وفقًا لثلاثة معايير: الاتساق الدلالي، والدقة النحوية، والطبيعية والطلاقة. "
        "أعطِ كل معيار درجة بين 0.0 و 5.0 بدقة منزل عشرية واحدة (مثل: 4.3). "
        "من فضلك، أجب بصيغة JSON فقط بدون أي شرح إضافي.\n"
        "مثال:\n{\"الاتساق الدلالي\": 5.0, \"الدقة النحوية\": 2.5, \"الطبيعية والطلاقة\": 4.3}"
    )

    prompt = (
        f"[|system|]{system_instruction}[|endofturn|]\n"
        f"[|user|]النص الأصلي (إنجليزي): {original_doc_text}\n\nالترجمة (عربي): {translated_doc_text}[|endofturn|]\n"
        f"[|assistant|]"
    )
...
#     system_instruction = (
#         "Avalie a qualidade da tradução entre o texto original em inglês e a tradução em português "
#         "com base em três critérios: consistência semântica, precisão gramatical e naturalidade e fluidez. "
#         "Atribua uma pontuação para cada critério entre 0.0 e 5.0 com uma casa decimal. "
#         "Responda apenas no formato JSON, sem explicações ou comentários adicionais. "
#         "Exemplo: {\"Consistência semântica\": 5.0, \"Precisão gramatical\": 2.5, \"Naturalidade e fluidez\": 4.3}"
#     )

#     prompt = (
#         f"[|system|]{system_instruction}[|endofturn|]\n"
#         f"[|user|]Texto original (inglês): {original_doc_text}\n\nTradução (português): {translated_doc_text}[|endofturn|]\n"
#         f"[|assistant|]"
#     )
...
#     semantic_score = float(scores.get("Consistência semântica", 0))
#     grammar_score = float(scores.get("Precisão gramatical", 0))
        ...
        for query_idx, (orig_doc_group, trans_doc_group, query_text) in tqdm(
            ...
            for i, (orig_doc_text, trans_doc_text) in enumerate(zip(orig_doc_group, trans_doc_group)):
                ...
                if is_non_arabic(orig_doc_text):
                    ...
                    # final_text = (
                    #     f"{trans_doc_text}\n"
                    #     f"[점수] 의미론적 일치성: {semantic_score}, 문법적 정확성: {grammar_score}, 자연스러움과 유창성: {fluency_score}"
                    # # )
                    final_text = (
                        f"{trans_doc_text}\n"
                        f"[التقييم] الاتساق الدلالي: {semantic_score}, الدقة النحوية: {grammar_score}, الطبيعية والطلاقة: {fluency_score}"
                    )
                    # final_text = (
                    #     f"{trans_doc_text}\n"
