...
class RAG:
    ...
    def eval(self, dataset_split):
        ...
        print("gen_dataset column names:", gen_dataset.column_names)
        if 'doc' not in gen_dataset.column_names:
            raise ValueError("gen_dataset does not contain 'doc'. Check retrieval/rerank steps or input data.")
        
        gen_dataset = self.translate_docs(gen_dataset)

        # print("\n[DEBUG] Translated docs preview:")
        for i, doc in enumerate(gen_dataset['doc'][:3]):
            print(f"Doc {i+1}: \n{doc}\n")
